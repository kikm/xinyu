...
public class getBaiduPhoto implements Runnable{
	    String link;
	    int i;  //x坐标
	    int j;  //y坐标
	    int z;  //缩放级别
	    ...
	    public getBaiduPhoto(String link, int i, int j, int z) {
	        this.link = link;
	        this.i = i;
	        this.j = j;
	        this.z = z;
	    ...
	    public static void main(String[] args)
	        ...
	        for (int i = xmin; i <= xmax; i++) {   //循环X
	            for (int j = ymin; j <= ymax; j++) {    //循环Y
	                threadPoolExecutor.execute(new getBaiduPhoto(link,i,j,z));
